...
public class SpecDetailFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
       ...
       binding = DataBindingUtil.inflate(inflater, R.layout.fragment_spec_detail,
               container, false);
       setUpUi();
    ...
    private void setUpUi(){
        ...
        binding.sendEventButton.setOnClickListener(v -> {
    ...
    private void identify(Events events){
        Analytics.with(getContext()).identify(new Traits().putName(events.getTraits().getName())
                .putEmail(events.getTraits().getEmail()).putGender(events.getTraits().getGender())
                .putAge(events.getTraits().getAge()));

    }
    private void track(Events events){
        Analytics.with(getContext()).track(events.getEvent(),
    ...
    private void screen(Events event){
        ...
        Analytics.with(getContext()).screen(event.getName(),
                new Properties().putValue("Feed Length", "26"));
    ...
    private void group(Events event){
        Analytics.with(getActivity()).group(event.getGroupId(),
                new Traits().putEmployees(event.getTraits().getEmployees())
